# ==============================================================================
# ROBUST CAUSAL MEDIATION (INTERVENTIONAL DIRECT / INDIRECT EFFECTS)
...
#
# Robustness features:
#   - Filter only communities with n>=100, plus pre-justified exclusion of C3_1
#   - Stable rare-category lumping for baseline categorical covariates (explicit "Missing")
#   - Explicit missing handling (numerics: median + flag)
...
KFOLDS        <- 3
MC_DRAWS      <- 50     # Increase to 500-1000 for final
BOOT_REPS     <- 50     # Increase to 500-1000 for final
...
df_viz <- all_data %>%
  ...
  dplyr::filter(Is_C2) %>%
  tidyr::pivot_longer(
    cols = c(TE, IDE, IIE, TE_lo, TE_hi, IDE_lo, IDE_hi, IIE_lo, IIE_hi),
    names_to = c("Effect", ".value"),
    names_pattern = "^(TE|IDE|IIE)(?:_(lo|hi))?$"
  ) %>%
  dplyr::mutate(
